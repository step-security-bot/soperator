...
func (r Reconciler) reconcile(
	...
	reconcileImpl := func() error {
		...
		err := r.EnsureDeployed(ctx, cluster, existingCronJob, desired, deps...)
		if err != nil {
			logger.Error(err, "Failed to deploy")
			return errors.Wrap(err, "deploying")
		}

		patch, err := r.patch(existingCronJob, desired)
		if err != nil {
			logger.Error(err, "Failed to patch")
			return errors.Wrap(err, "patching")
		}
		err = r.EnsureUpdated(ctx, cluster, existingCronJob, desired, patch, deps...)
		if err != nil {
...
func (r Reconciler) patch(_, _ client.Object) (client.Patch, error) {
